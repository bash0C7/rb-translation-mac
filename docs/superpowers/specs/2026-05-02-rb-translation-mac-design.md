...
Date: 2026-05-02
Status: Approved (2026-05-02). Implementation tracked via writing-plans output.
...
## Background research (2026-05-02, updated post-PoC)

- `TranslationSession` cannot be instantiated directly on the project's target platform. `init(installedSource:target:)` is `@available(macOS 26, *)` only (verified by smoke test 2026-05-02). Therefore session is delivered only through the `.translationTask(_:action:)` SwiftUI view modifier.
- `LanguageAvailability` is a plain class: `await languageAvailability.supportedLanguages`, `await languageAvailability.status(from:to:)`. Returns `.installed | .supported | .unsupported`. Works without SwiftUI.
- First-time translation triggers a system sheet asking the user to download the language model. **PoC verified**: the sheet attaches to the helper's invisible 1×1 borderless window AND macOS promotes it to the foreground so the user sees it. No visible window required.
- **Translation framework requires macOS 15.0+** (Apple docs say 14.4+, but the compiler reports `TranslationSession` and `.translationTask` as `@available(macOS 15.0, *)` only — verified by smoke test 2026-05-02). Other 5 sibling gems target `.macOS(.v12)`.
- Translation framework is **not TCC-gated** (no `NSTranslationUsageDescription` plist key needed).
- Swift 6 strict concurrency rejects `await session.translate(...)` from nonisolated contexts (`session` is main-actor-isolated). Helper must opt into Swift 5 language mode (`swiftSettings: [.swiftLanguageMode(.v5)]`) on the executable target.
...
│       ├── (TranslationMac-Swift.h auto-generated by swift_gem 6.3, gitignored)
│       └── Sources/
│           ├── TranslationMac/                 # lightweight tier
│           │   ├── TranslationMacBridge.swift  # @c (SE-0495): translation_mac_supported_languages, _status
...
- `Package.swift`: `platforms: [.macOS(.v15)]`, `swift-tools-version: 6.0` (matches swift_gem 6.3 scaffold)
- `gemspec`: no enforcement (no clean way), document in README
- README "Requirements" section to state macOS 15.0+ and Swift 6.3+ (swift_gem requirement)
...
| 5 | macOS version | `.macOS(.v15)` (15.0+) | Translation framework requirement, verified by smoke test 2026-05-02. Apple docs claim 14.4+ but `TranslationSession` and `.translationTask` are `@available(macOS 15.0, *)`. |
| 6 | Swift concurrency mode | Swift 5 mode for helper target | `session` is main-actor-isolated; nonisolated `await session.translate` triggers Swift 6 strict-concurrency `[#SendingRisksDataRace]`. Set `swiftSettings: [.swiftLanguageMode(.v5)]` on the helper `.executableTarget`. |
| 7 | First-run UX | invisible window OK | PoC verified: macOS promotes the language-model download sheet to foreground regardless of the parent window's visibility. |
